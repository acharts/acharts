...
describe('测试生成', function() {
  ...
  describe('测试生成',function(){
    it('测试生成项个数',function(){
      ...
      expect(series.getItems().length).to.be(series.get('data').length);
    });
    it('测试生成项的宽度',function(){
      
      expect(series.get('columnWidth')).to.be(group.get('xAxis').getTickAvgLength()/2);
    });
    it('测试生成的label',function(){
      expect(series.get('labelsGroup')).not.to.be(undefined);
      expect(series.get('labelsGroup').getCount()).to.be(series.getItems().length);
    });
    it('测试个性化项',function(){
      var item = series.getItems()[2];
      expect(item.get('point').yValue).to.be(20.1);
...
describe('测试柱状图宽度', function() {
  ...
  describe('测试柱状图宽度',function(){
    it('起始宽度',function(){
      ...
      expect(first.get('columnWidth')).to.be(9.375);
    });

    it('隐藏一个',function(done){
       var first = group.getSeries()[0],
        second = group.getSeries()[1],
        preWidth = second.get('columnWidth');
      group.hideSeries(first);
      setTimeout(function(){
         expect(second.get('columnWidth') ).not.to.be(preWidth);
